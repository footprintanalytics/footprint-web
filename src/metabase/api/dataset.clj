...
(api/defendpoint ^:streaming POST "/"
  ...
  [:as {{:keys [database fga-schema type project-role power-level] :as query} :body}]
  ...
  (let [result (-> query
                    ...
                    (update-in [:middleware :project-role] (fnil identity project-role))
                    (update-in [:middleware :power-level] (fnil identity power-level)))]
