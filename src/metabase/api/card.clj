...
(api/defendpoint ^:streaming POST "/:card-id/query"
  ...
  [card-id :as {{:keys [parameters ignore_cache dashboard_id collection_preview fga-schema web-vip], :or {ignore_cache false dashboard_id nil}} :body}]
  ...
  (qp.card/run-query-for-card-async
   ...
   :middleware   {:process-viz-settings? false :fga-schema fga-schema :web-vip web-vip}))
...
(api/defendpoint ^:streaming POST "/:card-id/query/:export-format"
  ...
  [card-id export-format :as {{:keys [parameters max-results fga-schema web-vip]} :params}]
