...
export class FieldDimension extends Dimension {
  ...
  field(): {} {
    if (this.isIntegerFieldId()) {
      let table = this._query?.table();
      const joinAlias = this.joinAlias();
      if (typeof joinAlias === "string") {
        if (joinAlias?.includes("Question")) {
          const aliasArray = this.joinAlias()?.split(" ");
          if (aliasArray.length > 1) {
            const tableId = aliasArray[1];
            table = this._metadata?.tables[`card__${tableId}`];
          }
        } else if (joinAlias === "source") {
          const key = Object.keys(this._metadata?.fields).find(
            id => id?.split("-")[0] === `${this._fieldIdOrName}`,
          );
          const aliasArray = key?.split("-");
          if (aliasArray.length > 1) {
            const tableId = aliasArray[1];
            table = this._metadata?.tables[`card__${tableId}`];
          }
        } else {
          table = Object.values(this._metadata?.tables).find(
            ({ name }) => name === joinAlias,
          );
        }
      }
      ...
      const findMetadataField = Object.values(
        this._metadata?.fields || [],
      ).find(({ id }) => id === this._fieldIdOrName);
      return (
        (this._metadata && findMetadataField) ||
        new Field({
          id: this._fieldIdOrName,
  ...
  render(): string {
    let displayName = location.pathname.includes("/chart/buffet")
      ? this.columnName()
      : this.displayName();
    ...
    if (this.fk()) {
      ...
      let tableName = this.joinAlias();
      if (
        tableName &&
        tableName.startsWith("Question") &&
        tableName.split(" ").length > 1
      ) {
        const chartId = tableName.split(" ")[1];
        tableName =
          this._metadata?.table(`card__${chartId}`)?.displayName() || tableName;
      }
      displayName = `${tableName} ${FK_SYMBOL} ${displayName}`;
    }
