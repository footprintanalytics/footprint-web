...
export default class Question {
  ...
  setDefaultDisplay(newGuide): Question {
    if (this.displayIsLocked()) {
      return this;
    }
    if (newGuide) {
  ...
  _syncStructuredQueryColumnsAndSettings(previousQuestion, previousQuery) {
    ...
    if (
      ...
      // Solving the problem of field duplication
      return this.updateSettings({
        "table.columns": uniqWith(
          [
            ...this.setting("table.columns"),
  ...
  async reduxCreate(dispatch) {
    const createMethod = this.card()?.create_method || "self"; //new_guide, template, preview
