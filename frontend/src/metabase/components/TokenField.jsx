...
export default class TokenField extends Component {
  ...
  _updateFilteredValues = (props: Props) => {
    ...
    if (optionDefaultFirstKey) {
      filteredOptions = filteredOptions.sort((o1, o2) => {
        if (o1.includes(optionDefaultFirstKey)) {
          return -1;
        }
        if (o2.includes(optionDefaultFirstKey)) {
          return 1;
        }
        return 0;
      });
      ...
      filteredOptions.length === 0 ? null : (
        ...
        >
          {filteredOptions.map((option, index) => (
            <li
              className="mr1"
              key={this._key(option)}
              id={`token-field-option-${index}`}
            >
              <div
                className={cx(
