            ...
            {this.props.list.filter(c => c.id !== user.personal_collection_id)
              ...
              <Box className="text-centered">
                ...
                <h3 className="text-medium">
                  {user.is_superuser
                    ? t`Save dashboards, queries, and collections in "${ROOT_COLLECTION.name}"`
                    : t`Access dashboards, queries, and collections in "${ROOT_COLLECTION.name}"`}
                </h3>
              </Box>
            )}
            <Link
