  ...
  for (const suggestion of syntacticSuggestions) {
    ...
    if (nextTokenType === Identifier || nextTokenType === IdentifierString) {
      ...
      if (isDimension) {
        ...
        if (customColsMap) {
          Object.values(customColsMap).forEach(item => {
            finalSuggestions.push({
              type: "fields",
              name: item,
              text: `[${item}]`,
      ...
      finalSuggestions.push(
        ...functions
          .filter(clause => database.hasFeature(clause.requiresFeature))
          .map(clause =>
            functionSuggestion("functions", clause.name, true, clause.mode),
          ),
      );
...
function functionSuggestion(type, clause, parens = true, mode) {
