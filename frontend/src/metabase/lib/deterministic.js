...
export function deterministicAssign(
  ...
  const otherColors = secondaryTiers[0];
  const primaryLen = primaryTier.length;
  const secondLen = otherColors.length;

  const unassigned = new Set(keys.slice()); // sort the keys for extra determinism
  const values = [...primaryTier, ...otherColors];
  ...
  while (unassigned.size > 0) {
    for (const key of unassigned) {
      if (existing[key]) {
        assign(key, existing[key]);
      } else {
        let value;
        if (index > primaryLen) {
          value = otherColors[(index - primaryLen) % secondLen];
        } else {
          value = primaryTier[index % primaryLen];
        }
        assign(key, value);
      }
    }
