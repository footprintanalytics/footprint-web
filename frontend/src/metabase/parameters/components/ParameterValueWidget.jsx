...
export default class ParameterValueWidget extends Component {
  ...
  updateFieldValues(props) {
    ...
    const useChainFilter = isCreator()
      ? dashboard && (dashboard.entityId || dashboard.id)
      : dashboard && dashboard.id;
    if (!useChainFilter) {
      for (const id of getFieldIds(props.parameter)) {
        props.fetchField(id);
        props.fetchFieldValues(id, getFieldTableId(props.parameter, id));
      }
    }
  }
...
function getFieldTableId(parameter, id) {
  const { fields } = parameter;
  return fields?.find(f => f?.id === id)?.table_id;
}
