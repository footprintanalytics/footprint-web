...
export const getMetadata = createSelector(
  ...
  (databases, schemas, tables, fields, segments, metrics): Metadata => {
    ...
    meta.fields = copyObjectByFields(meta, fields, instantiateField);
...
export function copyObjectByFields(metadata, objects, instantiate) {
  const copies = {};
  for (const object of Object.values(objects)) {
    if (object && object.id != null) {
      const id = `${object.id}-${object.table_id || ""}`;
