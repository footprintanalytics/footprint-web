...
import { MetabaseApi, CardApi, UserApi, PublicApi } from "metabase/services";
...
export const CARD_SECRET_DATA = "metabase/card/CARD_SECRET_DATA";
export const cardSecretData = createAction(
  CARD_SECRET_DATA,
  async ({ uuid, secret }) => {
    const { data } = await PublicApi.cardSecret({ uuid, secret });
    return data;
  },
);
...
export const redirectToNewQuestionFlow = createThunkAction(
  ...
  () => (dispatch, getState) => dispatch(replace("/chart/new")),
);
