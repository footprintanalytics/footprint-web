...
const TagEditorHelp = ({
  ...
  return (
    <div className="px3 text-spaced">
      ...
      <p>
        {t`Giving a variable the "Field Filter" type allows you to link queries to dashboard filter widgets or use more types of filter widgets on your SQL question. A Field Filter variable inserts SQL similar to that generated by the GUI query builder when adding filters on existing columns.`}
      </p>
      <p>
        {t`When adding a Field Filter variable, you'll need to map it to a specific field. You can then choose to display a filter widget on your query, but even if you don't, you can now map your Field Filter variable to a dashboard filter when adding this query to a dashboard. Field Filters should be used inside of a "WHERE" clause.`}
      </p>
      <TagExample
