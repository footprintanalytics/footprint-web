  ...
  const columns = [
    {
      ...
      dataIndex: 'name',
      key: 'name',
      render: (text) => <Link to={`/fga/game/project/${projectName}/journey`}>{text}</Link>,
    },
    {
      title: 'Description',
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="middle">
          <Link onClick={() => router.push(`/fga/game/project/${projectName}/journey-edit`)}><Icon name="edit_document" size={12} color="white"/></Link>
          <a onClick={() => {
